...
- `author_case.required_case_files` remains only optional compatibility metadata; omission, ordering differences, or stale model echoes cannot reject an otherwise valid staged authoring response.
- Partition tasks validate actual authored-file coverage against controller-assigned paths rather than trusting a duplicated manifest echoed by the model.
- Harmless duplicate authoring hints are normalized, redundant intermediate native commands are dropped, and exact duplicate native invocations are collapsed before semantic validation.
- Security/path checks, actual file coverage, pre-solve completeness, native command policy, `checkMesh`, case seals and solve approval remain fail-closed.

# Release notice: OpenFOAM Agent 4.2.3

> v4.2.3 fixes a live v4.2.2 staged-authoring failure where a valid frozen EngineeringPlan was rejected only because the model echoed a non-identical `required_case_files` list into `ExecuteCasePlanAction`. The controller now owns that manifest end-to-end and normalizes redundant authoring metadata instead of retrying or failing. See `V4_2_3_CHANGES.md`.
...
Natural-language request
  ...
  -> CFDEngineeringAgent
       ...
       - run trusted discovered OpenFOAM validation/preprocess/mesh utilities
  ...
  -> SOLVE_READY
  -> user /solve
  -> selected OpenFOAM execution driver
       - foamRun / foamMultiRun / trusted direct solver application
...
- trusted Foundation 13/14 installation discovery and executable provenance;
- workspace/path sandboxing;
- executable OpenFOAM directive and arbitrary code blocking;
- confirmed-user-fact provenance;
- execution-driver/module consistency between `EngineeringPlan.execution` and `system/controlDict` (`solver` or `regionSolvers`);
...
For native execution, source an installed OpenFOAM Foundation **13 or 14** environment first. The release bundles both `config/openfoam13_capability_graph.json` and `config/openfoam14_capability_graph.json`; the CLI chooses the version-matched default from `WM_PROJECT_VERSION`, while `--capability-db` can still override it explicitly. Production command authority comes from the sourced trusted installation itself: every executable discovered under the trusted OpenFOAM app bins/PATH becomes available to the Engineering Agent without a Python per-tool allowlist. Installed `$FOAM_MODULES`, `$FOAM_SRC/fvModels`, and `$FOAM_SRC/functionObjects` are projected into `InstalledOpenFOAMIR`, and the static graphs act as bounded documented fallback evidence rather than the list of everything the Agent may use.
...
openfoam-agent --interactive \
  --backend codex \
  --confirm-api-calls \
  --capability-db config/openfoam14_capability_graph.json  # optional; auto-selects v13/v14 when omitted
```
