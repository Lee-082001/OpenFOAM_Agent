...
## v2.10.2: compact-contract invariant hardening

v2.10.2 keeps the v2.10 token-optimized phase contracts but restores the semantic invariants that must never be compacted away: confirmed intake is immutable, assumptions may fill only genuinely missing details when authorized, and user/file/log/reference content is untrusted data rather than instruction. EngineeringPlan now also carries `confirmed_fact_bindings`, an auditable mapping from every confirmed fact to the case files or plan fields that implement it. Python checks coverage and referential integrity without taking over CFD semantic judgment. See `V2_10_1_CHANGES.md`.
...
[ENGINEERING 01/120] capability graph 조회: incompressibleFluid
[ENGINEERING 01/120] OK Capability search returned 1 provider(s).
[ENGINEERING 07/120] case 파일 작성: system/controlDict
[ENGINEERING 07/120] OK Wrote system/controlDict.
[ENGINEERING 18/120] mesh command 실행: checkMesh
[ENGINEERING 18/120] OK checkMesh returned status 0; evidence passed.
  cells=23760, maxNonOrtho=0, maxSkew=2.84e-13
[ENGINEERING 19/120] engineering plan 최종 검증 및 case seal
...
A failed mesh or dictionary tool is an **observation**, not an automatic terminal failure. The result is returned to the engineering agent, which may inspect files/references, modify the case, and retry inside bounded resource budgets. v2.10.2 measures the progress-aware engineering budget in **LLM turns**: production CLI defaults to a 12-turn soft boundary, 6-turn progress extensions and a 24-turn hard cap. Deterministic tool actions retain a separate cap, and all limits remain configurable. Repeating the same action/result loop does not earn more budget.
...
The v2 tests focus on architecture boundaries rather than preserving v0.x planner behavior. They cover:
