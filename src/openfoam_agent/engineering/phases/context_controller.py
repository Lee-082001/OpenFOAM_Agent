...
from openfoam_agent.contracts.evidence import (
    implementation_evidence_pack,
    advisory_authoring_evidence_summary,
)
from openfoam_agent.contracts.evidence_policy import POLICY_SUMMARY, provider_is_sufficient
from openfoam_agent.engineering.authoring_tasks import compile_tasks
...
from openfoam_agent.engineering.repair_context import (
    ...
    direct_repair_evidence_is_sufficient,
...
from openfoam_agent.engineering.phases.repair_episode import ensure_episode
...
def generate_turn(
    ...
    *,
    ...
    if contract_phase == "author_case":
        ...
        observed_support = [
            compact_event_for_model(event, excerpt_chars=700, summary_chars=280)
            for event in self._current_round_events(state)[-12:]
            if event.success and event.action_type in support_types
        ][-4:]
        episode_projection = project_repair_episode(episode)
        direct_repair_first = repair_episode_requires_direct_attempt(episode)
        repair_retrieval_policy = dict(retrieval_policy)
        if direct_repair_first:
            repair_retrieval_policy.update({
                "available": False,
                "disabled_reason": "native_diagnostic_explicit_alternatives_direct_repair_first",
                "mode": "direct_repair_first",
                "reopen_after_direct_attempt_or_new_failure": True,
            })
        payload = {
            ...
            "repair_episode_projection": episode_projection,
            "validation_failure": (episode_projection or {}).get("current_failure"),
            "relevant_case_files": relevant_files,
            "case_file_contract_scan": self._runtime_case_file_contract_scan(state),
            "case_inventory": case_files[:24],
            "mesh_evidence": {
                "passed": bool(state.mesh_evidence and state.mesh_evidence.passed),
                "cell_count": state.mesh_evidence.cell_count if state.mesh_evidence else None,
                "max_non_orthogonality": state.mesh_evidence.max_non_orthogonality if state.mesh_evidence else None,
            ...
            "supporting_observations": observed_support,
            "evidence_retrieval_policy": repair_retrieval_policy,
            "native_diagnostic_sufficiency": {
                "explicit_supported_alternatives": direct_repair_evidence_is_sufficient(
                    (episode_projection or {}).get("current_failure"), relevant_files
                ),
            ...
            "repair_contract": {
                ...
                "evidence_archive_is_not_prompt_memory": True,
                "rule": (
                    "Fix the observed case validation failure, not the whole design. "
                    "When native_diagnostic_sufficiency.direct_repair_first is true, choose a repair "
                    "from the native-supported alternatives using the implicated file(s) before requesting "
                    "reference retrieval. Python reports sufficiency but never chooses the CFD value. "
        ...
        instruction = (
            ...
            "actual OpenFOAM pre-solve consumer after the delta. When evidence_retrieval_policy.mode is "
            "direct_repair_first, do not request reference/capability retrieval on this turn; make one direct "
            "repair attempt from the supplied native alternatives and implicated file evidence. Retrieval may "
            "reopen after that attempt or after a new native syntax gap:\n"
        )
    elif use_delta:
        payload = {
