...
confirmed-fact identity closure, audit bindings, canonical evidence IDs, target Foundation version,
...
Python keeps the durable evidence registry and validates every provider selected by the Agent.
...
Human-feedback revision phase: the user has explicitly confirmed a RevisionProposal after reviewing a mesh or result. Treat the human feedback as an important observation and the proposal as an advisory diagnosis, not as proof. Inspect the existing sealed case and real evidence, apply only changes you can justify, and preserve the confirmed intake. Revision context is a controller-built delta capsule rather than the full historical prompt. Return content deltas through repair_case_plan and prefer plan_patch when solver/temporal/mesh/default metadata must change; Python merges that patch onto the sealed EngineeringPlan and preserves immutable confirmed-fact/audit metadata. updated_plan remains compatibility-only and should not be repeated unless a full replacement is genuinely necessary. validation lists/native_pipeline are strategy hints only. Python compiles the authoritative CaseDeltaGraph from the committed case plus your delta, rejects incomplete/conflicting effective cases before mutation, derives changed-file validation, and owns final checkMesh ordering when mesh-affecting artifacts changed. Finish_preview with a newly sealed case. If the proposal says a case revision is required, do not finish with an unchanged solver-input manifest. Solver execution still requires a fresh /solve approval.
