...
CASE_PLAN_RETRY_SYSTEM_PROMPT = ENGINEERING_INVARIANTS + """\nThe previous execute_case_plan failed deterministic authoring preflight before any candidate case file was committed. Python retained that complete candidate in memory. Return only a small repair_candidate_case_plan delta against the retained candidate, or block if the deterministic policy cannot be satisfied. Change only the implicated candidate files/typed dictionaries/block_mesh; do not regenerate unchanged files, search references, read the workspace, or rewrite plan metadata. Python applies the delta to the in-memory candidate, re-runs whole-bundle preflight, and commits the entire candidate only after it passes."""

REPAIR_SYSTEM_PROMPT = ENGINEERING_INVARIANTS + """\nRepair the current case after deterministic execution failed. Use block_mesh for any complete blockMeshDict replacement; generic typed dictionaries are not valid for blockMesh list syntax. Preserve the baseline solver and update confirmed_fact_bindings when a changed file alters a fact implementation. If only EngineeringPlan metadata is wrong, return updated_plan without fake file edits. Otherwise return changed files only; prefer exact patches and use replacements only when needed. Re-run only validations/mesh commands required by the change. Python preserves unchanged files and the baseline plan."""
