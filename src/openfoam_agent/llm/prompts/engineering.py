...
CASE_PLAN_RETRY_SYSTEM_PROMPT = ENGINEERING_INVARIANTS + """\nThe previous execute_case_plan failed deterministic authoring preflight before any candidate case file was committed. The CFD decision is already made. Use the exact diagnostic to return one corrected complete execute_case_plan, preserving all unchanged engineering choices and files from the previous plan; or block if the deterministic policy cannot be satisfied. Do not search references, read files, or issue delta repair actions: there is intentionally no partial case to repair. Python will preflight the entire replacement bundle again before the first write."""
